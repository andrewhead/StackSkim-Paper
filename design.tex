...
From then on, any time they select text for that language, an explanation will appear in a tooltip overlaid on the document, directly beneath the source (Figure~\ref{fig:explain_on_select}).   \marti{Why not have an option to have everything automagically recognized, no matter what the type, rather than making me specify every type?  And then let me say, e.g., "no more sql" when the occasion arises?}
