...
After receiving explanations, an explanation will appear in a tooltip overlaid on the document directly beneath the code example any time the programmer selects an explainable string of text (see Figure~\ref{fig:accelerators}). \marti{Why say "After receiving explanations, and explanation will appear?}
...
Our current tutorons extract blocks of code from \texttt{<code>}, \texttt{<pre>}, \texttt{<div>}, and \texttt{<p>} elements. \marti{why lower case tutoron?}
...
Note that if this can be combined with the parsing stage of the \Gls{name} processing pipeline. \marti{Sentence fragment}
...
\begin{changes}
It may happen that the same explainable region is detected by multiple tutorons. \marti{why lower case?}
...
\end{changes}
\marti{lower case tutoron in paragraph above}
...
Method 2: Develop a custom parser for the language that supports a subset of the language.
\begin{changes}
