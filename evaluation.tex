...
We can achieve a 100\% success rate parsing valid wget commands by reusing the parser from the wget source code.
...
Of 466 selectors, 440 (94\%) parsed without error.
Of the 26 that failed to parse, 13 of them included incorrectly-formatted characters in the original HTML markup or jQuery-specific pseudo-classes that do not belong to the CSS syntax.
The remaining 13 selectors illuminated cases our parser cannot yet handle: selector groups (multiple selectors joined by commas), double quotes around attribute values, and pseudo-classes that take arguments (e.g., \texttt{:nth-child(5)}).
Altogether, we achieve high parsing success rate for a concise hand-written parser, and will augment the selector \Gls{name} to properly parse the above cases in an upcoming revision.
