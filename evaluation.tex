...
We collected a set of programming help documents that the language of each of our \Glspl{name}. \marti{Fix this sentence.}
...
We manually extracted the element and character location of all uses of the language from 50 randomly selected tutorials, yielding more than 100 examples per language. \marti{Randomly selected from where?  From this set you just queried?}  We ran the \Glspl{name} on each document and manually evaluated extracted regions. \marti{check this rewording} 
...
We built our CSS selector parser by hand, and discuss its accuracy on real selectors from the web here. \marti{Not clear why the earlier test was not real and not the web.}
