...
\centering{%
    ...
    Note that we bolden the font of the selected node, for example in subplot~\ref{fig:html_table}, where the selected element in the \texttt{td} nested inside a \texttt{tr} in a \texttt{table}.\andrew{Shorten width, show only b---d, on same line.}}\label{fig:selector_demonstrations}
}
\end{figure}
...
\caption{Examples of strings we generate to match regular expressions.\andrew{Shorten this to one row per regex}}
