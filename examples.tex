...
In an early prototype, we developed a \Gls{name} for visualizing regular expressions, leveraging the third-party regular expression visualizer called \emph{RegExper} (Figure~\ref{fig:regex_visualization}).
...
First, leverage multiple representations to illuminate high-level intent and enable low-level understanding of syntax.
...
To determine common argument groups that can be described together, a first-revision \Gls{name} can be used to detect and parse code examples from online Q\&As or web tutorials.
